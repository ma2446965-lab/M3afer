...
- ✅ Lectures marketplace (`/lectures`): recorded video lectures sold **per-lecture** (`{product:"lecture"}`) or as a **whole-subject bundle at 20% off** (`{product:"lecture-bundle"}` — item list + price recomputed server-side from Firestore). Video URL + notes PDF live in `lectures/{id}/private/media` — readable ONLY by admin, purchasers (`lecturePurchases/{lectureId}_{uid}` webhook-created), or anyone for free/preview lectures. YouTube Unlisted via youtube-nocookie embeds. Admin tab «المحاضرات 🎬» (create/publish/price/delete/uploads + sales counts). Thumbnails `lecture-thumbs/` (public), notes `lecture-notes/` (gated) — both in `storage.rules`.
  - **Public catalog**: guests browse titles/prices without an account (rules already allow it); only purchasing/watching requires sign-in. Auth pages honor a sanitized `?next=` (`lib/nav.ts → sanitizeNextPath`, open-redirect safe) so a guest who taps "buy" lands right back on that lecture after login.
- ✅ **Messaging «رسائل 💬»** (`/messages`): realtime chats on `conversations/{convId}` + create-only `messages` subcollection.
  - **dm** = subscriber ↔ subscriber (rules `get()`-verify BOTH users docs; deterministic id `dm_<sortedUids>`). Peer discovery is **exact-uuid only** — deliberately no public student directory: students via `GET /api/users/lookup?uuid=` (returns a privacy-safe label, never the email), admins query Firestore directly.
  - **support** = admin ↔ ANY user (incl. non-subscribers), opened by the admin only — from `/messages` or the «💬 مراسلة» button in the students table (`/messages?support=<uid>` auto-opens the room `support_<uid>`). Admin messages inside support render as the platform identity **«مِعافر 🤖»** (`fromPlatform: true`, an admin-only privilege in rules).
  - Non-subscribers can't start dms but CAN reply inside support rooms (rules + UI gate with an upgrade CTA). 2000-char client cap / 4000-byte rules ceiling. Two-pane at lg+, single-pane with back below.
- ✅ **Courses «الكورسات 📦» — data model + admin (PR A of 3)**: lectures group into courses.
  - `courses/{id}`: public catalog doc (title/subject/teacher/desc/thumb/discountPct 0–0.5/published/order + denormalized `lectureCount`), admin-shape-guarded in rules. `lectures.courseId` links a lecture to exactly one course.
  - **Ownership = fan-out** (same as bundles): buying a course grants every member lecture in `lecturePurchases` via the webhook; `coursePurchases/{courseId}_{uid}` is a server-only audit receipt. Course price is **computed** (`lib/courses.ts computeCourseQuote`) — never stored, never client-supplied.
  - `lectureProgress/{lectureId}_{uid}`: student-owned watched state (grants nothing; rules whitelist incl. `lastPositionSec` reserved for resume-v2).
  - Admin tab «الكورسات 📦»: create/edit courses (thumb → existing `lecture-thumbs/`), attach/detach lectures, ↑↓ reorder inside a course, automatic `lectureCount` recount, sales counts. LecturesManager gained a course dropdown on create + per-row reassign + «بدون كورس» badge.
- ✅ **Courses storefront (PR B of 3) «الكورسات ✨»**: public `/courses` catalog (guests browse — payment/login only on buy; live subject filters/search, ownership strip «عندك 3 من 8», computed quote + «وفّر Xج» savings chip) and `/courses/[id]` detail (ownership progress bar, discounted whole-course buy CTA with post-payment auto-confirm polling, per-lecture rows: owned ▶ / free-preview ببلاش / locked price). Checkout gains `product:"course"` (server-recomputed quote — `course_not_found/unpublished/lookup_failed`, `nothing_to_buy` 409; client never sends a price). Webhook gains `handleCoursePaid`: idempotent fan-out grant of every published+paid member lecture into `lecturePurchases` (grants now also carry `courseId`) + one `coursePurchases` audit receipt per purchase — all inside a single transaction, retry-safe by deterministic doc ids. Cross-links: `/lectures` cards show a non-blocking «📦 من كورس» chip (page survives before the courses rules block is published), `/lectures/[id]` links «شوف الكورس كامل بخصم». Nav gained «الكورسات ✨» (SideNav + drawer). Students' progress UI (PR C) follows.
- 🔧 **Mobile-nav regression fixes**: (1) hamburger drawer rendered empty on phones — `backdrop-blur-lg` on the AppShell top bar turned the header into the containing block for the `fixed inset-0` drawer overlay, squeezing it into the 56px header box (measured 332×55); the overlay now renders through `createPortal(..., document.body)`. (2) bottom pill — active item's `scale-105` painted over neighbours at ≤390px widths and a fixed `mb-3` ignored iOS `safe-area-inset-bottom`; scale removed, padding `px-4`, margin-bottom `max(0.75rem, env(safe-area-inset-bottom))`. Guarded by a Playwright geometry+interaction suite (pill containment/overlap/spacing at 360/390/414, drawer open→navigate→close→usable at 390/360, desktop sidebar unaffected at 768/1280).
...
- 💳 **Payment result pages**: dedicated «payment-success / payment-failed / payment-pending» (Fatorak redirectionUrls point here now) — open-redirect-safe ?next= hand-off back to the product page carrying its ?payment= state (banner + auto-confirm polling unchanged). Success auto-returns after a 5s countdown; failed reassures «مفيش فلوس اتخصمت» + support link; pending explains auto-activation. Payments stay on Next API routes + Vercel env secrets (server-only); switch staging↔production via FATORAK_BASE_URL (staging.fawaterk.com ↔ app.fawaterk.com) + matching keys — zero code edits.
