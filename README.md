...
- ✅ **Courses «الكورسات 📦» — data model + admin (PR A of 3)**: lectures group into courses.
  ...
  - Admin tab «الكورسات 📦»: create/edit courses (thumb → existing `lecture-thumbs/`), attach/detach lectures, ↑↓ reorder inside a course, automatic `lectureCount` recount, sales counts. LecturesManager gained a course dropdown on create + per-row reassign + «بدون كورس» badge. Student-facing `/courses` (PR B) + progress UI (PR C) follow.
