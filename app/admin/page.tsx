...
function FullScreenState({ children }: { children: React.ReactNode }) {
  return (
    <div className="min-h-screen flex items-center justify-center p-4 text-center bg-slate-50 dark:bg-navy-900">
      {children}
    </div>
  );
}
...
function AdminPageInner() {
    ...
    return (
      <FullScreenState>
        <div>
          ...
          <p className="mt-3 text-slate-500 dark:text-slate-400">جاري التحقق من تسجيل الدخول...</p>
        </div>
      </FullScreenState>
    );
  }
  ...
  if (!user) {
    ...
    return (
      <FullScreenState>
        <p className="text-slate-500 dark:text-slate-400">مش مسجل دخول — جاري تحويلك لصفحة تسجيل الدخول...</p>
      </FullScreenState>
    );
  }
  ...
  if (profileError) {
    return (
      <FullScreenState>
        <div className="max-w-md w-full bg-white dark:bg-navy-800 rounded-2xl border dark:border-navy-700 p-6 text-right">
          <AlertTriangle className="mx-auto text-accent-500" size={36} />
          ...
          <div className="text-xs text-slate-500 dark:text-slate-400 mt-4 space-y-1.5 leading-relaxed">
            <p className="font-bold text-slate-600 dark:text-gray-300">راجع النقاط دي بالترتيب:</p>
          ...
          <button
            onClick={handleRetry}
            disabled={retrying}
            className="mt-5 w-full bg-brand-600 hover:bg-brand-700 text-white font-bold py-2.5 rounded-xl flex items-center justify-center gap-2 disabled:opacity-50"
          >
  ...
  if (!profile) {
    ...
    return (
      <FullScreenState>
        <div>
          <Loader2 className="animate-spin mx-auto text-brand-500" size={32} />
          <p className="mt-3 text-slate-500 dark:text-slate-400">جاري تحميل ملفك الشخصي...</p>
          <button onClick={handleRetry} disabled={retrying} className="mt-3 text-sm text-brand-600 underline disabled:opacity-50">
  ...
  if (profile.role !== "admin") {
    ...
    return (
      <FullScreenState>
        <p className="text-slate-500 dark:text-slate-400">الصفحة دي للأدمن بس 🔒 — جاري تحويلك للرئيسية...</p>
      </FullScreenState>
    );
  }

  return (
    <div className="min-h-screen bg-slate-50 dark:bg-navy-900 p-4 pt-8 pb-16">
      <div className="max-w-5xl mx-auto">
            ...
            <button
              ...
              className={`flex items-center gap-2 px-5 py-2.5 rounded-xl font-bold text-sm whitespace-nowrap transition-colors ${
                tab === id
                  ...
                  : "bg-white dark:bg-navy-800 border dark:border-navy-700 text-slate-600 dark:text-gray-300 hover:border-gray-300"
              }`}
            >
              <Icon size={16} /> {label}
            </button>
        ...
        <div className="mt-6 p-4 bg-accent-50 dark:bg-accent-900/10 rounded-xl border border-accent-100 dark:border-accent-900/20">
          <p className="text-xs text-accent-800 dark:text-accent-200 leading-relaxed">
