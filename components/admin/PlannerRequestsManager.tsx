...
function RequestCard({ r, onFulfilled }: { r: any; onFulfilled: () => void }) {
  ...
  return (
    <div className={`bg-white dark:bg-gray-800 rounded-2xl border-2 overflow-hidden ${overdue ? "border-red-300 dark:border-red-800" : "border-gray-100 dark:border-gray-700"}`}>
      ...
      {open && (
        <div className="px-4 pb-4 space-y-3 border-t dark:border-gray-700 pt-3">
          ...
          <div className="flex flex-wrap items-center gap-2 text-[11px]">
            <button onClick={copyUuid} className="font-mono bg-gray-100 dark:bg-gray-700 px-2 py-1 rounded-lg flex items-center gap-1">
              <Copy size={11} /> {copied ? "اتنسخ ✅" : r.studentUuid || r.studentId}
            </button>
            <span className="text-gray-400">{r.studentEmail}</span>
            {(intake.grade || intake.track) && (
              <span className="bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-300 px-2 py-1 rounded-lg">
            ...
            <span className="bg-teal-50 dark:bg-teal-900/30 text-teal-600 dark:text-teal-300 px-2 py-1 rounded-lg">
          ...
          {Array.isArray(intake.weekly) && intake.weekly.length > 0 && (
            <div className="text-xs space-y-1.5">
              <p className="font-bold text-gray-500">الجدول الأسبوعي:</p>
              {intake.weekly.map((d: any) => (
                <div key={d.day} className="bg-gray-50 dark:bg-gray-700/50 rounded-lg p-2">
                  <b>{d.day}</b>
                  {d.busy && <p className="text-gray-500">🔒 مشغول: {d.busy}</p>}
                  {d.free && <p className="text-gray-500">🟢 فاضي: {d.free}</p>}
                </div>
              ))}
            </div>
          )}
          {intake.prioritySubjects?.length > 0 && (
            <p className="text-xs text-gray-500"><b>أولويات:</b> {intake.prioritySubjects.join("، ")}</p>
          )}
          {intake.upcomingExams && <p className="text-xs text-gray-500"><b>امتحانات قريبة:</b> {intake.upcomingExams}</p>}
          {intake.notes && <p className="text-xs text-gray-500"><b>ملاحظات:</b> {intake.notes}</p>}
          {r.payment?.invoiceId != null && (
            <p className="text-[11px] text-gray-400 font-mono">invoice: {String(r.payment.invoiceId)} • {r.payment.method || ""}</p>
          )}
          ...
          {r.status === "fulfilled" ? (
            ...
            <div className="bg-teal-50/60 dark:bg-teal-900/10 border border-teal-100 dark:border-teal-900/30 rounded-xl p-3 space-y-2">
              ...
              <input
                ...
                className="text-xs w-full file:mr-2 file:py-1.5 file:px-3 file:rounded-lg file:border-0 file:bg-teal-500 file:text-white file:font-bold"
              />
              <input
                value={note}
                onChange={(e) => setNote(e.target.value)}
                placeholder="ملاحظة للطالب (اختياري)"
                className="w-full p-2 rounded-lg border dark:border-gray-600 bg-white dark:bg-gray-700 text-xs outline-none"
              />
              {err && <p className="text-[11px] text-red-500 flex items-center gap-1"><AlertTriangle size={12} /> {err}</p>}
              <button
                onClick={send}
                disabled={!file || busy}
                className="w-full bg-teal-600 hover:bg-teal-700 disabled:opacity-50 text-white font-bold py-2.5 rounded-xl flex items-center justify-center gap-2 text-sm"
              >
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h2 className="font-bold flex items-center gap-2">
          <CalendarDays size={18} className="text-teal-500" /> طلبات «جدولي» المدفوعة
        </h2>
        <button onClick={() => setTick((t) => t + 1)} className="p-2 rounded-lg bg-gray-100 dark:bg-gray-700" title="تحديث">
          <RefreshCw size={14} />
        </button>
      </div>
        ...
        {FILTERS.map((f) => (
          <button
            ...
            className={`text-xs font-bold px-3 py-1.5 rounded-full whitespace-nowrap ${
              filter === f.id ? "bg-teal-600 text-white" : "bg-gray-100 dark:bg-gray-700 text-gray-500"
            }`}
          >
